...
class testWalkthrough(unittest.TestCase):
    ...
    def testWmediumdAdhoc(self):
        ...
        p.expect('1 packets transmitted, 1 received')
        p.expect(self.prompt)
        p.sendline('sta2 ping -c 1 sta3')
        p.expect('1 packets transmitted, 1 received')
        p.expect(self.prompt)
        p.sendline('sta1 ping -c 1 sta3')
        p.expect('1 packets transmitted, 0 received')
        p.expect(self.prompt)
        p.sendline('sta1 ping6 -c1 fe80::2%sta1-wlan0')
        p.expect('1 packets transmitted, 1 received')
        p.expect(self.prompt)
        p.sendline('sta2 ping6 -c1 fe80::3%sta2-wlan0')
        p.expect('1 packets transmitted, 1 received')
        p.expect(self.prompt)
        p.sendline('sta1 ping6 -c1 fe80::3%sta1-wlan0')
        p.expect('1 packets transmitted, 0 received')
        p.expect(self.prompt)
        p.sendline('exit')
        p.wait()
